// Package parser contains the parser for Monkey lang
package parser
...
func New(l *lexer.Lexer) *Parser {
	...
	// Done so that peekToken and curToken are both set
	p.nextToken()
	p.nextToken()
...
func (p *Parser) registerPrefix(
	tokenType token.TokenType, fn prefixParseFn,
) {
	p.prefixParseFns[tokenType] = fn
}

func (p *Parser) registerInfix(
	tokenType token.TokenType, fn infixParseFn,
) {
	p.infixParseFns[tokenType] = fn
}

// peekError adds an error to parser errors
...
func (p *Parser) parseArrayLiteral() ast.Expression {
	exp := &ast.ArrayLiteral{Elements: []ast.Expression{}}
