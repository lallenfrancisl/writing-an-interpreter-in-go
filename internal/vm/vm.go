...
func NewWithGlobalsStore(
	bytecode *compiler.Bytecode, s []object.Object,
) *VM {
	vm := New(bytecode)
	vm.globals = s
...
func (vm *VM) Run() error {
	for ip := 0; ip < len(vm.instructions); ip++ {
		...
		case code.OpJump:
			...
			// INFO: Storing after reducing 1 since we are in a loop
			ip = pos - 1
...
func (vm *VM) executeBinaryOperation(
	op code.Opcode,
) error {
	right := vm.pop()
	left := vm.pop()
...
func (vm *VM) executeBinaryStringOperation(
	...
	return vm.push(
		&object.String{Value: leftValue + rightValue},
	)
}

func (vm *VM) executeComparison(
	op code.Opcode,
) error {
	right := vm.pop()
	left := vm.pop()
