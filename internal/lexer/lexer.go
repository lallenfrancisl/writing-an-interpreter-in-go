// Package lexer contains the lexer for monkey lang
package lexer
...
// TODO: Try adding support for unicode characters
...
func (l *Lexer) NextToken() token.Token {
	...
	case '=':
		if l.peekChar() == '=' {
			...
			tok = *token.NewToken(token.EQ, l.ch)
	...
	case '!':
		if l.peekChar() == '=' {
			...
			tok = *token.NewToken(token.NOT_EQ, l.ch)
			tok.Literal = string(ch) + string(l.ch)
		} else {
	...
	default:
		if isLetter(l.ch) {
			...
			tok = *token.NewToken(token.LookupIdent(literal), 0)
			tok.Literal = literal

			return tok
		} else if isDigit(l.ch) {
			tok = *token.NewToken(token.INT, 0)
